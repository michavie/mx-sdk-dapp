...
function getTransactionsByHashes(pendingTransactions) {
  return __async(this, null, function* () {
    ...
    const { data: responseData } = yield axios3.get(`${networkConfig.network.apiAddress}/transactions`, {
      params: {
        hashes: hashes.join(","),
        withScResults: true
      }
...
// src/hooks/transactions/useTrackTransactionStatus.ts
function useTrackTransactionStatus({
  transactionId: txId,
  onSuccess,
  onFail,
  onCancelled,
  ...
  const isWalletProvider = getIsProviderEqualTo("wallet" /* wallet */);
  const [lastSessionId] = signedTransactionsArray.length > 0 ? signedTransactionsArray[signedTransactionsArray.length - 1] : [];
  const walletSessionId = (_a = txId != null ? txId : lastSessionId) != null ? _a : null;
  const transactionId = isWalletProvider ? walletSessionId : txId;
  const transactionsBatch = useSelector((state) => transactionStatusSelector(state, transactionId));
...
function useGetFailedTransactions() {
  ...
  const failedTransactionsArray = Object.entries(failedTransactions);
  const hasFailedTransactions = (failedTransactionsArray == null ? void 0 : failedTransactionsArray.length) > 0;
  return {
    failedTransactions,
...
function useGetSuccessfulTransactions() {
  ...
  const successfulTransactionsArray = Object.entries(successfulTransactions);
  const hasSuccessfulTransactions = (successfulTransactionsArray == null ? void 0 : successfulTransactionsArray.length) > 0;
  return {
...
function useGetActiveTransactionsStatus() {
  ...
  const successfulTransactions = useSelector(successfulTransactionsSelector);
  const pendingTransactions = useSelector(pendingSignedTransactionsSelector);
  const pending = ((_a = Object.keys(pendingTransactions)) == null ? void 0 : _a.length) > 0;
  const timedOut = !pending && ((_b = Object.keys(timedOutTransactions)) == null ? void 0 : _b.length) > 0;
  const fail = !pending && !timedOut && ((_c = Object.keys(failedTransactions)) == null ? void 0 : _c.length) > 0;
