...
// src/wrappers/AuthenticatedRoutesWrapper/index.tsx
init_shim();
...
function getNetworkConfigFromProxyProvider() {
  return __async(this, null, function* () {
    try {
      return yield getProxyProvider().getNetworkConfig();
    } catch (err) {
...
// src/wrappers/AuthenticatedRoutesWrapper/index.tsx
var AuthenticatedRoutesWrapper = ({
  children,
  routes,
  unlockRoute,
  onRedirect
  ...
  const authenticatedRoutesRef = useRef(routes.filter((route) => Boolean(route.authenticatedRoute)));
  const isOnAuthenticatedRoute = authenticatedRoutesRef.current.some(({ path }) => pathname === path);
  const shouldRedirect = isOnAuthenticatedRoute && !isLoggedIn && walletLogin == null;
...
var AuthenticatedRoutesWrapper_default = AuthenticatedRoutesWrapper;

// src/wrappers/AppInitializer.tsx
init_shim();
...
function sendSignedTransactions(signedTransactions) {
  return __async(this, null, function* () {
    const { apiAddress, apiTimeout } = networkSelector(store.getState());
    const promises = signedTransactions.map((transaction) => {
      return axios.post(`${apiAddress}/transactions`, transaction.toPlainObject(), { timeout: parseInt(apiTimeout) });
    });
...
function getTransactionsByHashes(pendingTransactions) {
  return __async(this, null, function* () {
    const networkConfig = networkConfigSelector(store.getState());
    const hashes = pendingTransactions.map((tx) => tx.hash);
    const { data: responseData } = yield axios2.get(`${networkConfig.network.apiAddress}/transactions`, {
      params: {
        hashes: hashes.join(","),
        withScResults: true
      }
    ...
    return pendingTransactions.map(({ hash, previousStatus }) => {
      const txOnNetwork = responseData.find((txResponse) => (txResponse == null ? void 0 : txResponse.txHash) === hash);
      return {
        hash,
...
function getServerConfiguration(apiAddress) {
  return __async(this, null, function* () {
    const cleanApiAddress = apiAddress.endsWith("/") ? apiAddress.slice(0, -1) : apiAddress;
    const configUrl = `${cleanApiAddress}/${configEndpoint}`;
    try {
      const { data } = yield axios3.get(configUrl);
      if (data != null) {
        return data;
      }
    } catch (err) {
...
function getPendingTransactions(transactions, timedOutHashes) {
  const pendingTransactions = transactions.reduce((acc, { status, hash }) => {
...
var getPendingTransactions_default = getPendingTransactions;
...
function manageFailedTransactions({
  ...
  const resultWithError = results.find((scResult) => (scResult == null ? void 0 : scResult.returnMessage) !== "");
...
var manageFailedTransactions_default = manageFailedTransactions;
...
var manageTimedOutTransactions_default = manageTimedOutTransactions;
...
function manageTransaction({
  ...
  const { hash, status, results, invalidTransaction, hasStatusChanged } = serverTransaction;
  try {
    ...
    if (retriesForThisHash > 30) {
      manageTimedOutTransactions_default(sessionId);
      return;
    }
    ...
    if (hasStatusChanged) {
      if (getIsTransactionSuccessful(status) && (customTransactionInformation == null ? void 0 : customTransactionInformation.completedTransactionsDelay) != null) {
        timeouts.push(hash);
        setTimeout(() => store.dispatch(updateSignedTransactionStatus({
          sessionId,
    ...
    if (getIsTransactionFailed(status)) {
      manageFailedTransactions_default({ sessionId, hash, results });
    }
  } catch (error) {
    console.error(error);
    manageTimedOutTransactions_default(sessionId);
...
function checkBatch(_0) {
  return __async(this, arguments, function* ({
    ...
    try {
      ...
      const pendingTransactions = getPendingTransactions_default(transactions, timeouts);
      const serverTransactions = yield getTransactionsByHash(pendingTransactions);
...
var checkBatch_default = checkBatch;

// src/hooks/transactions/useCheckTransactionStatus/useCheckTransactionStatus.ts
function useCheckTransactionStatus() {
  const { pendingTransactionsArray } = useGetPendingTransactions();
  function checkTransactionStatus(props) {
    return __async(this, null, function* () {
      const pendingBatches = pendingTransactionsArray.filter(([sessionId, transactionBatch]) => {
        const isPending = sessionId != null && getIsTransactionPending(transactionBatch.status);
        return isPending;
      });
      if (pendingBatches.length > 0) {
        for (const [sessionId, transactionBatch] of pendingBatches) {
          yield checkBatch_default(__spreadValues({
            sessionId,
            transactionBatch
          }, props));
        }
...
// src/utils/account/getAccountShard.tsx
init_shim();
  ...
  return __async(this, null, function* () {
    ...
    try {
      if (shard == null && address) {
        const { data: account } = yield axios6.get(`${apiAddress}/accounts/${address}`);
...
function AppInitializer({
  ...
  function initializeNetwork() {
    return __async(this, null, function* () {
      const fetchConfigFromServer = !(customNetworkConfig == null ? void 0 : customNetworkConfig.skipFetchFromServer);
      const customNetworkApiAddress = customNetworkConfig == null ? void 0 : customNetworkConfig.apiAddress;
      const fallbackConfig = fallbackNetworkConfigurations[environment] || {};
      const localConfig = __spreadValues(__spreadValues({}, fallbackConfig), customNetworkConfig);
      if (fetchConfigFromServer) {
        const fallbackApiAddress = fallbackConfig == null ? void 0 : fallbackConfig.apiAddress;
        const serverConfig = yield getServerConfiguration(customNetworkApiAddress || fallbackApiAddress);
        if (serverConfig != null) {
          const apiConfig = __spreadValues(__spreadValues(__spreadValues({}, fallbackConfig), serverConfig), customNetworkConfig);
  ...
  useEffect9(() => {
    if (address) {
      const pubKey = new Address10(address).hex();
      if (pubKey !== publicKey) {
        logout();
      }
    }
  ...
  return initialized ? /* @__PURE__ */ React4.createElement(React4.Fragment, null, children) : null;
}
var AppInitializer_default = AppInitializer;
...
import { ExtensionProvider as ExtensionProvider4 } from "@elrondnetwork/erdjs-extension-provider";
import { HWProvider as HWProvider3 } from "@elrondnetwork/erdjs-hw-provider";
function ProviderInitializer() {
  ...
  const isLoggedIn = useSelector(isLoggedInSelector);
  const [ledgerData, setLedgerData] = useState10();
  const dispatch = useDispatch();
  const { callbackRoute, logoutRoute } = walletConnectLogin ? walletConnectLogin : { callbackRoute: "", logoutRoute: "" };
  ...
  function refreshChainID() {
    getNetworkConfigFromProxyProvider().then((networkConfig) => {
      if (networkConfig) {
    ...
    return __async(this, null, function* () {
      try {
        if (walletLogin != null) {
          ...
          const address2 = yield getAddress();
          if (address2) {
            setAccountProvider(provider);
            dispatch(loginAction({ address: address2, loginMethod: "wallet" /* wallet */ }));
            const account = yield getAccount(address2);
            if (account) {
              dispatch(setAccount({
                balance: account.balance.toFixed(),
                address: address2,
  ...
  function getInitializedHwWalletProvider() {
    return __async(this, null, function* () {
      const hwWalletP = new HWProvider3();
      let isInitialized = hwWalletP.isInitialized();
      if (!isInitialized) {
        isInitialized = yield hwWalletP.init();
      }
  ...
  function setLedgerProvider() {
    return __async(this, null, function* () {
      try {
        const hwWalletP = yield getInitializedHwWalletProvider();
        if (!hwWalletP) {
          return;
        }
        const ledgerConfig = yield getLedgerConfiguration(hwWalletP);
  ...
  function setExtensionProvider() {
    return __async(this, null, function* () {
      try {
        const address2 = yield getAddress();
        const provider = ExtensionProvider4.getInstance().setAddress(address2);
        const success = yield provider.init();
        if (success) {
          setAccountProvider(provider);
        } else {
...
// src/wrappers/DappProvider/CustomComponents.tsx
init_shim();
...
var { logoutEvent } = localStorageKeys;
...
var LogoutListener_default = LogoutListener;

// src/components/TransactionSender/index.ts
init_shim();
import React5, { useRef as useRef5 } from "react";
import { Address as Address11 } from "@elrondnetwork/erdjs";
  ...
  function handleSendTransactions() {
    return __async(this, null, function* () {
      ...
      for (const sessionId of sessionIds) {
        const skipSending = (_b = (_a = signedTransactions == null ? void 0 : signedTransactions[sessionId]) == null ? void 0 : _a.customTransactionInformation) == null ? void 0 : _b.signWithoutSending;
        if (!sessionId || skipSending) {
          continue;
        }
        try {
          const isSessionIdSigned = signedTransactions[sessionId].status === "signed" /* signed */;
          ...
          const transactionsToSend = transactions.map((tx) => {
            const address = new Address11(tx.sender);
            const transactionObject = newTransaction_default(tx);
            const signature = new Signature(tx.signature);
            transactionObject.applySignature(signature, address);
            return transactionObject;
          });
          const responseHashes = yield sendSignedTransactionsAsync(transactionsToSend);
          const newStatus = "pending" /* pending */;
          const newTransactions = transactions.map((transaction) => {
            if (responseHashes.includes(transaction.hash)) {
              return __spreadProps(__spreadValues({}, transaction), { status: newStatus });
            }
            return transaction;
          });
...
var TransactionSender_default = TransactionSender;

// src/components/TransactionsTracker/index.tsx
init_shim();

// src/components/TransactionsTracker/useTransactionsTracker.ts
init_shim();
import { useEffect as useEffect12, useRef as useRef6 } from "react";
function useTransactionsTracker(props) {
  var _a;
  const { pendingTransactionsArray } = useGetPendingTransactions();
  const getTransactionsByHash = (_a = props == null ? void 0 : props.getTransactionsByHash) != null ? _a : getTransactionsByHashes;
...
var useTransactionsTracker_default = useTransactionsTracker;

// src/components/TransactionsTracker/index.tsx
function TransactionsTracker({
  getTransactionsByHash
}) {
  useTransactionsTracker_default({ getTransactionsByHash });
  return null;
}
var TransactionsTracker_default = TransactionsTracker;
...
function CustomComponents({
  ...
  var _a, _b;
  const transactionSender = customComponents == null ? void 0 : customComponents.transactionSender;
  const transactionTracker = customComponents == null ? void 0 : customComponents.transactionTracker;
  const TxSender = (_a = transactionSender == null ? void 0 : transactionSender.component) != null ? _a : TransactionSender_default;
  const TxTracker = (_b = transactionTracker == null ? void 0 : transactionTracker.component) != null ? _b : TransactionsTracker_default;
  return /* @__PURE__ */ React6.createElement(React6.Fragment, null, /* @__PURE__ */ React6.createElement(TxSender, __spreadValues({}, transactionSender == null ? void 0 : transactionSender.props)), /* @__PURE__ */ React6.createElement(TxTracker, __spreadValues({}, transactionTracker == null ? void 0 : transactionTracker.props)), /* @__PURE__ */ React6.createElement(LogoutListener_default, null));
}

// src/wrappers/DappProvider/DappProviderContext.ts
init_shim();
import React7 from "react";
import {
  createDispatchHook as createDispatchHook2,
  createSelectorHook as createSelectorHook2,
  createStoreHook as createStoreHook2
} from "react-redux";
var defaultContextValue2 = null;
var DappCoreContext2 = React7.createContext(defaultContextValue2);
var useStore2 = createStoreHook2(DappCoreContext2);
var useDispatch3 = createDispatchHook2(DappCoreContext2);
var useSelector2 = createSelectorHook2(DappCoreContext2);
...
var DappProvider = ({
  ...
  return /* @__PURE__ */ React8.createElement(Provider, {
    ...
    store
  }, /* @__PURE__ */ React8.createElement(PersistGate, {
    persistor,
    loading: null
  }, /* @__PURE__ */ React8.createElement(AppInitializer_default, {
    environment,
    customNetworkConfig
  }, /* @__PURE__ */ React8.createElement(ProviderInitializer, null), /* @__PURE__ */ React8.createElement(CustomComponents, {
    customComponents
  }), children)));
};
export {
  AppInitializer,
  AuthenticatedRoutesWrapper_default as AuthenticatedRoutesWrapper,
  DappProvider
};
/*!
